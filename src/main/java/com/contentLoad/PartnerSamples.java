...
public class PartnerSamples {
	...
	public boolean login() {
		...
		try {
			...
			System.out.println(partnerConnection.getSessionHeader()
					.getSessionId());
				...
				for (SObject contactVersion : lSobject) {
					...
					for (int j = 0; j < results.length; j++) {
						if (results[j].isSuccess()) {
							...
							System.out
									.println("\nA content version was created with an ID of: "
											+ result);
						} else {
	...
	public void createRest(List<StringBuilder> lJsonString, String sessionId)
			throws IOException {
		for (StringBuilder sbJson : lJsonString) {
			String result = "";
			HttpPost post = new HttpPost(
					"https://veck--dev.my.salesforce.com/services/data/v44.0/sobjects/ContentVersion");
	...
	public void callRestService(List<EntityContentVersion> lContentVersion,String sessionId){
		String response;
		for(EntityContentVersion entity:lContentVersion){
    	PostRequestBuilder req = new PostRequestBuilder(entity.getPathOnClient(), entity);
	    try {
				HttpService.uploadDoc(req.preparePostRequest(sessionId));
